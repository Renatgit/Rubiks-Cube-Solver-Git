...
public class CubeState : MonoBehaviour
{
    ...
    public static CubeStateData CreateSolvedState()
    {
        return new CubeStateData
        {
            ...
            //Alignments
            firstEightEdgesAlignment = new Dictionary<int, string>()
            {
                { 0, "U" },
                { 1, "U" },
                { 2, "U" },
            ...
            lastFourEdgeAlignment = new Dictionary<int, string>()
            {
                { 8, "F" },
                { 9, "F" },
                { 10, "B" },
    ...
    public static CubeStateData CloneState(CubeStateData state)
    {
        ...
        return new CubeStateData
        {
            ...
            lastFourEdgeAlignment = CloneDictionary(state.lastFourEdgeAlignment),
            firstEightEdgesAlignment = CloneDictionary(state.firstEightEdgesAlignment),
            depth = state.depth,
            solution = CloneList(state.solution)
        };
    }
    ...
    private static Dictionary<TKey, TValue> CloneDictionary<TKey, TValue>(Dictionary<TKey, TValue> dictionary)
    {
    ...
    private List<int> GetCornerPermutation()
        {
            ...
            // value = corner piece currently in that position
            for (int position = 0; position < 8; position++)
            {
                string cornerAtPosition = cornersNamesPermutation[position];

                for (int piece = 0; piece < 8; piece++)
                {
                    if (cornerAtPosition == correctCornerOrder[piece])
                    {
                        cornerPermutation.Add(piece);
                        break;
                    }
                }
            }
